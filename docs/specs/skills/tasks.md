...
- [x] 1. 搭建 Skill 包基础结构和类型定义
  - [x] 1.1 创建 `packages/lobster-u/src/types.ts`，定义所有共享类型（SkillManifest, SmokeTest, BenchmarkTest, RubricItem, SkillCategory, BenchmarkDimension, Agent API 接口类型）
    - _Requirements: 1_
  - [x] 1.2 创建 `packages/lobster-u/src/validator.ts`，实现 manifest.json 校验器（验证必填字段、semver 格式、依赖引用合法性）
    - _Requirements: 1_
  - [x] 1.3 更新 `packages/lobster-u/src/index.ts`，导出类型和工具函数
    - _Requirements: 1_
  - [x] 1.4 更新 `packages/lobster-u/package.json`，添加 `"./skills/*"` subpath export 和 files 配置
    - _Requirements: 1_

- [x] 2. 创建 Skill 模板生成器
  - 创建 `packages/lobster-u/scripts/create-skill.ts` 脚手架脚本，输入 skill name + category 自动生成目录结构（manifest.json, SKILL.md, knowledge/, strategies/, tests/）
  - _Requirements: 1_
...
- [x] 15. 实现 `academic-search` Skill
  - [x] 15.1 编写 manifest.json（dependencies: `@lobster-u/google-search: ^1.0.0`）+ SKILL.md
...
- [x] 16. 实现 `rewriter` Skill
  - [x] 16.1 编写 manifest.json（dependencies: `@lobster-u/summarizer: ^1.0.0`）+ SKILL.md
...
- [x] 17. 实现 `debugger` Skill
  - [x] 17.1 编写 manifest.json（dependencies: `@lobster-u/code-review: ^1.0.0`）+ SKILL.md
...
- [x] 18. 实现 `refactor` Skill
  - [x] 18.1 编写 manifest.json（dependencies: `@lobster-u/code-review: ^1.0.0`）+ SKILL.md
...
- [x] 19. 实现 `doc-gen` Skill
  - [x] 19.1 编写 manifest.json（dependencies: `@lobster-u/code-gen: ^1.0.0`）+ SKILL.md
...
- [x] 20. 实现 `writer` Skill
  - [x] 20.1 编写 manifest.json（dependencies: `@lobster-u/summarizer: ^1.0.0`, `@lobster-u/keyword-extractor: ^1.0.0`）+ SKILL.md
...
- [x] 21. 实现 `copywriter` Skill
  - [x] 21.1 编写 manifest.json（dependencies: `@lobster-u/sentiment-analyzer: ^1.0.0`）+ SKILL.md
...
- [x] 22. 实现 `social-media` Skill
  - [x] 22.1 编写 manifest.json（dependencies: `@lobster-u/copywriter: ^1.0.0`）+ SKILL.md
...
- [x] 25. 构建与发布准备
  - 运行 `pnpm build` 验证 lobster-u 包构建成功
  - 验证 subpath exports（`lobster-u/skills/*`）正确解析
  - 更新 `packages/lobster-u/README.md` 文档：Skill 列表、安装方式、接口说明
  - _Requirements: 1_
